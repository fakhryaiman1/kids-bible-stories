...
import { BookOpen, Sparkles, UserCircle2, Star, Home, Library, Bookmark, LayoutGrid, LogOut } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

export const PublicNavbar: React.FC<{ onOpenAuth: () => void }> = ({ onOpenAuth }) => {
        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden items-center gap-2 md:flex">
            ...
            <Link
              to="/categories"
              className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold transition-all ${
                isActive('/categories') ? 'bg-sky-500 text-white shadow-md shadow-sky-200' : 'text-slate-700 hover:bg-slate-100'
              }`}
            >
              <LayoutGrid size={16} /> الفئات
            </Link>
            <Link
              to="/favorites"
              className={`flex items-center gap-2 rounded-full px-4 py-2 text-sm font-bold transition-all ${
                isActive('/favorites') ? 'bg-sky-500 text-white shadow-md shadow-sky-200' : 'text-slate-700 hover:bg-slate-100'
              }`}
            >
          ...
          <div className="flex items-center gap-3">
            {user ? (
              ...
              >
                <Sparkles size={16} /> تسجيل الدخول / حساب جديد
              </button>
            )}
